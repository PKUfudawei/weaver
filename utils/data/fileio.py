...
def _read_hdf5(filepath, branches, load_range=None):
    ...
    if load_range is not None:
        ...
        stop = max(
            start + 1, math.trunc(load_range[1] * len(outputs[branches[0]])))
...
def _read_root(filepath, branches, load_range=None, treename=None):
    ...
    with uproot3.open(filepath) as f:
        if treename is None:
            treenames = set([k.decode('utf-8').split(';')[0]
                            for k, v in f.allitems() if getattr(v, 'classname', '') == 'TTree'])
            if len(treenames) == 1:
                treename = treenames.pop()
            else:
                raise RuntimeError('Need to specify `treename` as more than one trees are found in file %s: %s' % (
...
def _write_root(file, table, treename='Events', compression=-1, step=1048576):
    ...
    with uproot3.recreate(file, compression=compression) as fout:
        fout[treename] = uproot3.newtree(
            {k: v.dtype for k, v in table.items()})
        start = 0
        while start < len(list(table.values())[0]) - 1:
            fout[treename].extend({k: v[start:start + step]
